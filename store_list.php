...
//总记录数
$datacount=$con->RowsCount("select count(*) from #__store");
//总页数
...
//查询数据
$sql='select * from #__store order by id desc limit '.$start_id.','.$pagesize;
$data	=$con->select($sql,'');
//得到分页HTML
...
<table cellpadding="3" cellspacing="0">
	...
	<?php foreach($data as $k=>$v){?>
    	<tr class='list'>
        	<td align="center">
				<?php 
				echo "<a class='edit' href='store_edit.php?id={$v['id']}' title='点击进入修改'>{$v['id']}</a>";
				?>
			</td>
...
<p class="toadd"><a href="store_add.php">添加门店</a></p>
